...
bool CLandRegister::add(const std::string& city, const std::string& addr, const std::string& region, unsigned long long id)
{
    ...
    properties.push_back({city, addr, region, id, ""});
...
{
    std::vector<Property> sortedProperties = properties;
    std::sort(sortedProperties.begin(), sortedProperties.end(), [](const Property& a, const Property& b) {
...
size_t CLandRegister::findProperty(const std::string& city, const std::string& addr) const
{
    ...
    auto it = std::lower_bound(properties.begin(), properties.end(), temp,
                               [](const Property& a, const Property& b) {
                                   return std::tie(a.m_City, a.m_Addr) < std::tie(b.m_City, b.m_Addr);
                               });

    if (it != properties.end() && it->m_City == city && it->m_Addr == addr)
...
size_t CLandRegister::findProperty(const std::string& region, unsigned long long id) const {
    ...
    if (it != properties.end() && it->m_Region == region && it->m_ID == id)
        return std::distance(properties.begin(), it);
    else
        return properties.size();
}
